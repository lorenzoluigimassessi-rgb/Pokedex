    ...
    <div className="screen" style={{ background:'var(--bg-pokedex)', color:'var(--text-primary)' }}>
      ...
      <header style={{ padding:'48px 20px 12px', background:'#fff',
        ...
        <div style={{ marginTop:12 }}>
          <ProgressBar value={Math.min(hi, lo + 400) - lo + 1} total={Math.min(hi, lo + 400) - lo + 1}/>
        </div>
      </header>
      ...
      <div className="scroll" style={{ flex:1, padding:'4px 16px 120px', position:'relative', zIndex:1 }}>
        {view === 'grid' ? (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10 }}>
            {ids.map(id => <GridCard key={id} id={id} info={caught[id]} ready={false} onOpen={onOpenDetail}/>)}
          </div>
        ) : (
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {ids.map(id => <ListRow key={id} id={id} info={caught[id]} ready={false} onOpen={onOpenDetail}/>)}
          </div>
        )}
      </div>
      {/* browse only - no catch button */}
    </div>
  );
}
